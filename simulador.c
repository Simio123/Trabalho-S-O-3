...
typedef struct frametable_entry {
  ...
  /* quantidade de vezes que é referenciado */
...
int selectVictimLRU(void) {
  ...
  int i;
  int victim = 0;
  unsigned int min_last_reference_time = -1;

  for (i = 0; i < n_frames; i++) {
    if (frameTable[i].last_reference_time < min_last_reference_time) {
      min_last_reference_time = frameTable[i].last_reference_time;
      victim = i;
    }
  }
...
int selectVictimSC(void) {
  ...
  while(1){
	  if(frameTable[victim].referenced == 0){
		  return victim;
	  }
	  else{
		  frameTable[victim].referenced = 0;
...
int selectVictimLFU(void) {
   ...
   */
    ...
    int i;
    int victim = 0;
    unsigned int min_reference_num = -1;

    for (i = 0; i < n_frames; i++) {
        if (frameTable[i].reference_num < min_reference_num) {
            min_reference_num = frameTable[i].reference_num;
            victim = i;
        }
    }
  return victim;
...
int selectVictimMFU(void) {
   ...
   */
    ...
    int i;
    int victim = 0;
    unsigned int max_reference_num = 0;

    for (i = 0; i < n_frames; i++) {
        if (frameTable[i].reference_num > max_reference_num) {
            max_reference_num = frameTable[i].reference_num;
            victim = i;
        }
    }
  return victim;
...
int findPage(int frameToUse) {
  /* retorna o numero da página carregada numa frame que é dada
     como argumento;
     so é chamada para frames que tem uma pagina carregada
   */
